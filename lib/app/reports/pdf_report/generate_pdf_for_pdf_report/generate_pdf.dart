...
import 'package:mohamed_mekhemar/app/course_members/generate_pdf/save_file_mobile.dart';
import 'package:syncfusion_flutter_pdf/pdf.dart';

import '../model/pdf_report_in_detail_model.dart';

class GeneratePdfReportPDF {
  Future<List<int>> _readFontData() async {
    final ByteData bytes = await rootBundle.load('assets/fonts/dinfont.ttf');
    return bytes.buffer.asUint8List(bytes.offsetInBytes, bytes.lengthInBytes);
  }

  Future<void> generatePdfReport(seenList, unSeenList, activityName) async {
    late File file;
    ...
    grid = getGrid(seenList, unSeenList);
    ...
    await saveAndLaunchFile(bytes, 'PdfReport.pdf');
  }
...
//Create PDF grid and return
  PdfGrid getGrid(
    List<SeenData> seenList,
    List<NotSeenData> unSeenList,
  ) {
    //Create a PDF grid
    ...
    headerRow.cells[2].value = 'Number of views';
    headerRow.cells[2].stringFormat.alignment = PdfTextAlignment.center;
    headerRow.cells[2].stringFormat.textDirection =
        PdfTextDirection.rightToLeft;

    void addStudent(
        String studentId, String studentIdName, String views, PdfGrid grid) {
      final PdfGridRow row = grid.rows.add();
      row.cells[0].value = studentId;
      row.cells[1].value = studentIdName;
      row.cells[2].value = views;
    }

    //Add rows
    for (var i in seenList) {
      addStudent(i.userid, i.name, i.views, grid);
    }

    for (var i in unSeenList) {
      addStudent(i.userid, i.name, i.views.toString(), grid);
    }
